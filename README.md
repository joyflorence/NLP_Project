...
```mermaid
graph TD
    A["OFFLINE INDEXING PIPELINE<br>PDFs → Extract → Clean → Chunk → Embed → Vector Index + Metadata<br>(Batch Processing)"]
    B["ONLINE QUERY PATH (FastAPI Backend)<br>Query → Embed → Vector Search → Aggregate by Doc → Rank → API Response"]
    C["INTERACTIVE WEB UI (React + Vite)<br>Search Interface | Results Display | Admin Dashboard | Document Management"]
    A --> B --> C
```
